...
from Model import MapModel, LocalizationModel
...
def test_epoch(epoch, dataloader, model, loss_fn, device):
    ...
    with torch.no_grad():
        for batchii, (x, y) in enumerate(dataloader):
            ...
            data_values.append((loss.detach().cpu().item() / x.shape[0], x.detach().cpu().numpy(),
...
def train(params, device):
    ...
    train_set = MapDataset(**params['train']['dataset_params'], isTrain=True)
    test_set = MapDataset(**params['train']['dataset_params'], isTrain=False)
    ...
    model = MapModel(3 * train_set.time_window, 1, train_set.nx, train_set.nx).to(device)
